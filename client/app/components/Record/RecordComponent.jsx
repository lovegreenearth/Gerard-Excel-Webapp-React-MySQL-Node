...
class RecordComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    var expIn = localStorage.getItem('expiredIn');
  ...
  render() {
    ...
    const unitList = units.map((unit, index) => {      
        return (
          <div className="products" key={unit.id} onClick={() => this.unitSelect(index)}>
            {unit.unit_name}
          </div>
        );
    });
    ...
    const cabinetList = cabinets.map((cabinet, index) => {
      return (
        <div className="products" key={cabinet.id} onClick={() => this.cabinetSelect(index)}>
      ...
      <div className="container-fluid">
        <div className="well">
          ...
          <div className="mb-20"></div>
          {
            step == 4 ? <div className="productCount">
              ...
              <div className="permit-message">
                {
                  this.state.stockOut ? <h5 className="alert-message">here is not enough stock to execute this command, 
                  please check number as well as type</h5> : null
                }
                {
          ...
          <div className="goHome">
            <button className="btn btn-primary" onClick={() => this.inout()}>Go Home</button>
          </div>
        </div>
      </div>
    );
